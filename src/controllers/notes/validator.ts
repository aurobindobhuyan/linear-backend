...
export const getOneNote = (req: Request, res: Response, next: NextFunction) => {
  const schema = Joi.object().keys({
    noteId: JoiMongoObjectId().required(),
  });
  const { error: paramError } = schema.validate(req.params);
  if (paramError) {
    throw new ValidationFailed(undefined, paramError.details[0].message);
  }
  next();
};
...
export const updateNote = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (bodyError) {
    throw new ValidationFailed(undefined, bodyError?.details[0]?.message);
  }
  next();
};
